...
% Vary starting templates by changing which batches are included.
...
firstBatch = getOr(ops, {'CSBseed'}, 1);
% guarding against bad input from CSBseed
if firstBatch > Nbatch
    firstBatch = 1;
elseif firstBatch >= nskip 
    firstBatch = mod(firstBatch,nskip);
end
fprintf( 'firstBatch in extractTemplatesfromSnippets: %d\n', firstBatch);
...
for ibatch = firstBatch:nskip:Nbatch
